...
# Generate dates from now until the 95th percentile completion date
dates = pd.date_range(start=datetime.now().date(), end=percentile_95_date, freq='B')

# Create a DataFrame for the WIP simulation
wip_data = pd.DataFrame({'Date': dates, 'WIP': [best_wip] * len(dates)})

# Create a line chart using Plotly Express
fig = px.line(wip_data, x='Date', y='WIP',
              title=f'WIP Simulation until {percentile_95_date}',
